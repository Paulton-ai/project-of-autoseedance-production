...
function PricingPreview() {
  const plans = [
    { name: 'Free', label: 'Start creating', text: 'Try the core creation tools before choosing a paid plan.', points: ['50 starter credits', 'AI image generation', 'AI video generation'], featured: false },
    { name: 'Pro', label: 'For active creators', text: 'More room for regular image, video and short-form creation.', points: ['More monthly credits', 'Image + video generation', 'Reel Studio access'], featured: true },
    { name: 'Max', label: 'For high-volume work', text: 'Built for creators and teams producing content more frequently.', points: ['Highest credit allowance', 'Full creation toolkit', 'Priority for bigger workflows'], featured: false },
  ];
  return <section id="pricing" className="border-y border-border bg-gradient-to-r from-[#f3eef8] via-[#faf5f1] to-[#f5edf6] py-24 md:py-28"><div className="mx-auto max-w-6xl px-4"><SectionIntro eyebrow="Plans for different creation needs" title="Create more without guessing what comes next" description="Auto Seedance uses credits across its creation tools. Start free, then choose the plan that matches how much content you make."/><div className="mt-12 grid gap-5 md:grid-cols-3">{plans.map((plan, i)=><motion.div key={plan.name} initial={{opacity:0,y:22}} whileInView={{opacity:1,y:0}} viewport={{once:true,margin:'-60px'}} transition={{delay:i*.07}} whileHover={{y:-5}}><Card className={`relative h-full border-border bg-white/80 p-7 shadow-sm backdrop-blur ${plan.featured ? 'ring-2 ring-primary/35 shadow-lg' : ''}`}>{plan.featured && <Badge className="absolute -top-3 left-6 border-0 bg-primary text-primary-foreground">Most popular</Badge>}<div className="flex items-start justify-between gap-3"><div><div className="text-xs font-bold uppercase tracking-[.16em] text-primary">{plan.label}</div><h3 className="mt-2 font-display text-2xl font-bold">{plan.name}</h3></div><div className="grid size-10 place-items-center rounded-xl bg-primary/10 text-primary"><CreditCard className="size-5"/></div></div><p className="mt-4 text-sm leading-6 text-muted-foreground">{plan.text}</p><div className="mt-6 space-y-3">{plan.points.map(point=><div key={point} className="flex items-start gap-2 text-sm"><Check className="mt-0.5 size-4 shrink-0 text-primary"/>{point}</div>)}</div><Link to="/pricing" className="mt-7 inline-flex items-center gap-2 text-sm font-semibold text-primary hover:gap-3 transition-all">View full pricing <ArrowRight className="size-4"/></Link></Card></motion.div>)}</div></div></section>;
}
