...
import {
  ArrowRight, Check, Clapperboard, CreditCard, Image as ImageIcon, Layers3,
...
function PricingPreview() {
  const plans = [
    { name: 'Free', label: 'Start creating', text: 'Start with the core creation tools and 30 free credits.', credits: '30 credits', points: ['30 starter credits', 'AI image generation', 'AI video generation'], featured: false },
    { name: 'Basic', label: 'For regular creators', text: 'A practical credit allowance for creators making content every week.', credits: '500 credits/month', points: ['500 credits/month', 'AI image generation', 'AI video generation', 'Multiple AI models'], featured: false },
    { name: 'Standard', label: 'For growing output', text: 'More monthly credits plus priority generation and support.', credits: '1,600 credits/month', points: ['1,600 credits/month', 'AI image generation', 'AI video generation', 'Multiple AI models', 'Priority generation'], featured: false },
    { name: 'Pro', label: 'For active creators', text: 'The highest current plan for creators producing content frequently.', credits: '4,000 credits/month', points: ['4,000 credits/month', 'AI image generation', 'AI video generation', 'Multiple AI models', 'Fastest generation speed', 'Reel Studio access'], featured: true },
  ];
  return <section id="pricing" className="border-y border-border bg-gradient-to-r from-[#f3eef8] via-[#faf5f1] to-[#f5edf6] py-24 md:py-28"><div className="mx-auto max-w-6xl px-4"><SectionIntro eyebrow="Plans for different creation needs" title="Choose the plan that fits your content volume" description="Auto Seedance uses one credit balance across its creation tools. Start free, then upgrade when you need more monthly generation capacity."/><div className="mt-12 grid gap-5 sm:grid-cols-2 lg:grid-cols-4">{plans.map((plan, i)=><motion.div key={plan.name} initial={{opacity:0,y:22}} whileInView={{opacity:1,y:0}} viewport={{once:true,margin:'-60px'}} transition={{delay:i*.07}} whileHover={{y:-5}}><Card className={`relative h-full border-border bg-white/80 p-7 shadow-sm backdrop-blur ${plan.featured ? 'ring-2 ring-primary/55 shadow-xl shadow-primary/20 before:absolute before:-inset-px before:-z-10 before:rounded-2xl before:bg-primary/15 before:blur-xl' : ''}`}>{plan.featured && <Badge className="absolute -top-3 left-6 border-0 bg-primary text-primary-foreground shadow-lg shadow-primary/30">Most popular</Badge>}<div className="flex items-start justify-between gap-3"><div><div className="text-xs font-bold uppercase tracking-[.16em] text-primary">{plan.label}</div><h3 className="mt-2 font-display text-2xl font-bold">{plan.name}</h3></div><div className="grid size-10 place-items-center rounded-xl bg-primary/10 text-primary"><CreditCard className="size-5"/></div></div><p className="mt-4 text-sm leading-6 text-muted-foreground">{plan.text}</p><div className="mt-5 rounded-xl border border-primary/20 bg-primary/5 px-3 py-2 text-sm font-semibold text-primary">{plan.credits}</div><div className="mt-5 space-y-3">{plan.points.map(point=><div key={point} className="flex items-start gap-2 text-sm"><Check className="mt-0.5 size-4 shrink-0 text-primary"/>{point}</div>)}</div><Link to="/pricing" className="mt-7 inline-flex items-center gap-2 text-sm font-semibold text-primary hover:gap-3 transition-all">View full pricing <ArrowRight className="size-4"/></Link></Card></motion.div>)}</div></div></section>;
}
