...
function Landing() {
  ...
  return <div className="min-h-screen bg-background text-foreground"><Navbar /><Hero /><ToolOverview /><ReelStory /><HowItWorks /><CreatorOutcomes /><Comparison /><BlogSection posts={posts} /><FAQ /><CTA /><Footer /></div>;
}

function Hero() {
  return <section className="relative overflow-hidden pt-32 pb-20 md:pt-40 md:pb-28 grid-bg">
    ...
    <div className="mx-auto max-w-6xl px-4 text-center">
      ...
      <div className="mx-auto mt-8 grid max-w-4xl grid-cols-2 gap-3 md:grid-cols-4">
        {[['3','core creation tools'],['30 / 60 / 90s','reel lengths'],['4','video model choices in Reel Studio'],['1','place for the creation process']].map(([n,l]) => <div key={l} className="rounded-2xl border border-border bg-white/75 px-3 py-4 backdrop-blur"><div className="font-display text-xl font-bold md:text-2xl">{n}</div><div className="mt-1 text-[11px] leading-4 text-muted-foreground">{l}</div></div>)}
      </div>
    </div>
  </section>;
}
...
function ToolOverview() {
  ...
  return <section id="tools" className="py-24 md:py-28"><div className="mx-auto max-w-6xl px-4"><SectionIntro eyebrow="Three ways to create" title="Pick the tool that matches the job" description="You do not need to learn a complicated production stack. Use a focused generator for a single asset, or use Reel Studio when you want to turn an idea into a complete short-form video." />
    <div className="mt-12 grid gap-5 lg:grid-cols-3">{tools.map((tool, i) => <motion.div key={tool.title} initial={{ opacity: 0, y: 22 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: '-70px' }} transition={{ duration: .5, delay: i * .07 }}><Card className="group h-full overflow-hidden border-border bg-white p-0 shadow-sm transition duration-300 hover:-translate-y-1 hover:shadow-xl"><div className="relative border-b border-border bg-gradient-to-br from-primary/10 via-white to-secondary/10 p-7"><div className="flex items-center justify-between"><span className="rounded-full border border-border bg-white/80 px-2.5 py-1 text-[10px] font-bold tracking-[.15em] text-muted-foreground">{tool.label}</span><tool.icon className="size-6 text-primary" /></div><div className="mt-10 flex items-end justify-between"><div className="font-display text-4xl font-bold tracking-tight">{tool.stat}</div><div className="size-16 rounded-2xl border border-white/80 bg-white/80 shadow-sm" /></div></div><div className="flex h-full flex-col p-7"><h3 className="font-display text-xl font-bold">{tool.title}</h3><p className="mt-3 text-sm leading-6 text-muted-foreground">{tool.text}</p><div className="mt-6 space-y-2 text-sm">{['Fast browser-based creation','Creator-friendly controls','Export and keep your output'].map(x => <div key={x} className="flex gap-2"><Check className="mt-0.5 size-4 shrink-0 text-primary" />{x}</div>)}</div><Link to={tool.href} className="mt-7 inline-flex items-center gap-2 text-sm font-semibold text-primary group-hover:gap-3 transition-all">{tool.action}<ArrowRight className="size-4" /></Link></div></Card></motion.div>)}</div>
  </div></section>;
}
...
function ReelStory() {
  ...
  return <section id="reel-studio" className="overflow-hidden border-y border-border bg-white/60 py-24 md:py-28"><div className="mx-auto max-w-6xl px-4"><div className="grid items-center gap-12 lg:grid-cols-[.9fr_1.1fr]"><div><Badge variant="outline" className="bg-white"><Clapperboard className="mr-1.5 size-3.5 text-primary" />Idea → finished reel</Badge><h2 className="mt-5 font-display text-4xl font-bold tracking-tight md:text-5xl">Stop starting with a blank timeline.</h2><p className="mt-5 text-base leading-7 text-muted-foreground">Reel Studio is built for the creator who has an idea but does not want to spend hours moving between a script writer, voice tool, image generator, video generator and editor.</p><p className="mt-4 text-base leading-7 text-muted-foreground">Describe the content you want. The creation process can handle the script, scene structure, generated clips, voiceover, captions and final assembly so you can spend more time deciding <em>what</em> to publish.</p><Link to="/tools/reel-studio"><Button className="mt-7 btn-gradient border-0 text-white">Build a reel <ArrowRight className="ml-1.5 size-4" /></Button></Link></div><div className="relative"><div className="absolute -inset-6 rounded-[2rem] bg-primary/10 blur-2xl"/><div className="relative grid gap-3 sm:grid-cols-2">{steps.map(([num,title,text]) => <motion.div key={num} whileHover={{ y: -4 }} className="rounded-2xl border border-border bg-white p-5 shadow-sm"><div className="flex items-center justify-between"><span className="font-display text-2xl font-bold text-primary/40">{num}</span><span className="grid size-8 place-items-center rounded-full bg-primary/10 text-primary"><Check className="size-4"/></span></div><h3 className="mt-4 font-display font-bold">{title}</h3><p className="mt-2 text-xs leading-5 text-muted-foreground">{text}</p></motion.div>)}</div></div></div></div></section>;
}
...
function FAQ() {
  ...
  return <section className="py-24 md:py-28"><div className="mx-auto max-w-4xl px-4"><SectionIntro eyebrow="Questions creators ask" title="Everything you need to know before creating"/><Accordion type="single" collapsible className="mt-10">{items.map(([q,a],i)=><AccordionItem key={q} value={`item-${i}`}><AccordionTrigger className="text-left font-display font-semibold">{q}</AccordionTrigger><AccordionContent className="leading-7 text-muted-foreground">{a}</AccordionContent></AccordionItem>)}</Accordion></div></section>;
}

function CTA() { return <section className="px-4 pb-20 md:pb-28"><div className="mx-auto max-w-6xl overflow-hidden rounded-[2rem] border border-border bg-gradient-to-br from-primary/10 via-white to-secondary/10 p-8 text-center md:p-14"><div className="mx-auto max-w-3xl"><div className="mx-auto grid size-12 place-items-center rounded-2xl bg-primary text-primary-foreground shadow-lg"><Zap className="size-6"/></div><h2 className="mt-6 font-display text-4xl font-bold tracking-tight md:text-5xl">Have an idea? Make something from it.</h2><p className="mx-auto mt-4 max-w-2xl text-base leading-7 text-muted-foreground">Create an image, generate a video, or turn your next idea into a reel. Start with the tool that fits your content and build from there.</p><div className="mt-8 flex flex-wrap justify-center gap-3"><Link to="/tools/reel-studio"><Button size="lg" className="btn-gradient border-0 text-white">Create a Reel <ArrowRight className="ml-1.5 size-4"/></Button></Link><Link to="/tools/image"><Button size="lg" variant="outline">Explore AI Image Generator</Button></Link></div></div></div></section>; }

function SectionIntro({ eyebrow, title, description }: { eyebrow: string; title: string; description?: string }) { return <div className="mx-auto max-w-3xl text-center"><Badge variant="outline" className="bg-white">{eyebrow}</Badge><h2 className="mt-4 font-display text-4xl font-bold tracking-tight md:text-5xl">{title}</h2>{description && <p className="mt-4 text-base leading-7 text-muted-foreground md:text-lg">{description}</p>}</div>; }
