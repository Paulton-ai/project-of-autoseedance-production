      ...
      <main className="flex-1 pt-28 pb-20">
        <article className="mx-auto max-w-3xl px-4">
          ...
          <div className="flex flex-wrap items-center gap-2 text-xs text-muted-foreground">
            {active.category && (
              <Link to="/blog" className="inline-block font-semibold uppercase tracking-wider text-primary bg-primary/10 px-2 py-1 rounded-md hover:bg-primary/15">{active.category}</Link>
            )}
            ...
            {dateModified !== active.publishedAt && (
              <time dateTime={dateModified} className="inline-flex items-center gap-1"><Clock className="size-3.5" />Updated {modifiedLabel}</time>
            )}
